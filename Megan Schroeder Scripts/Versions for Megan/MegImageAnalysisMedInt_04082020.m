...
inDir = 'C:\Users\dplad\Desktop\For Doug analysis\IF dilation analysis_D12 gels\Females\Control\*.czi';
Folder = 'C:\Users\dplad\Desktop\For Doug analysis\IF dilation analysis_D12 gels\Females\Control\';
Channels = 2;
%BinWidth = 500;
...
for sumx = 1:Res
    for sumy = 1:Res
    Ch1_med(sumx,sumy) = median(Ch1(sumx,sumy,:));
...
Ch1_NEAR = Ch1_medd(Ch1_BW3); Ch1_NEAR_MEAN(f,1) = mean(Ch1_NEAR);
Ch1_FAR = Ch1_medd(~Ch1_BW3); Ch1_FAR_MEAN(f,1) = mean(Ch1_FAR);
Ch2_NEAR = Ch2_medd(Ch1_BW3); Ch2_NEAR_MEAN(f,1) = mean(Ch2_NEAR);
Ch2_FAR = Ch2_medd(~Ch1_BW3); Ch2_FAR_MEAN(f,1) = mean(Ch2_FAR);
