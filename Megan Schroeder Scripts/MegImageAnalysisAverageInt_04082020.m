...
for sumx = 1:Res
    for sumy = 1:Res
    Ch1_avg(sumx,sumy) = mean(Ch1(sumx,sumy,:));
    Ch2_avg(sumx,sumy) = mean(Ch2(sumx,sumy,:));
    if Channels>2, Ch3_avg(sumx,sumy) = mean(Ch3(sumx,sumy,:)); else end
    if Channels>3, Ch4_avg(sumx,sumy) = mean(Ch4(sumx,sumy,:)); else end
    end
end

Ch1_avgd = uint16(Ch1_avg);
Ch2_avgd = uint16(Ch2_avg);
if Channels>2, Ch3_avgd = uint16(Ch3_avg); else end
if Channels>3, Ch4_avgd = uint16(Ch3_avg); else end

%%%%%%%%%%%%%%%%%%%
Ch1_BW1 = imbinarize(Ch1_avgd);
Ch1_BW2 = imerode(Ch1_BW1,erodestrel);
Ch1_BW3 = imdilate(Ch1_BW2,dilatestrel);

Ch1_NEAR = Ch1_avgd(Ch1_BW3); Ch1_NEAR_MEAN(f,1) = mean(Ch1_NEAR);
Ch1_FAR = Ch1_avgd(~Ch1_BW3); Ch1_FAR_MEAN(f,1) = mean(Ch1_FAR);
Ch2_NEAR = Ch2_avgd(Ch1_BW3); Ch2_NEAR_MEAN(f,1) = mean(Ch2_NEAR);
Ch2_FAR = Ch2_avgd(~Ch1_BW3); Ch2_FAR_MEAN(f,1) = mean(Ch2_FAR);
